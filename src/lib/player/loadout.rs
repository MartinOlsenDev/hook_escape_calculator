...
impl Loadout {
    /// Given a number of living players, returns the bonus from Up the Ante
    pub fn ante_calculator(&self, living_count: u8) -> f64 {
        match &self.up_the_ante_status {
            Some(ref ante) => {
                let l = LuckContributor::from(ante);
                l.ante_coefficient().unwrap() * living_count as f64
            },
            None => 0.0
        }
    }
...
mod tests {
    ...
    fn example0() {
        ...
        assert_eq!(survivor.ante_calculator(3), 0.0);
    ...
    fn example1() {
        ...
        assert_eq!(survivor.ante_calculator(2), 0.06);
