...
use iced::widget::{combo_box, text, Column, row, checkbox, Text};
use iced::Element;
...
impl App {
    ...
    fn update_survivor(&mut self, survivor_update: SurvivorUpdate) {
        let player = self.team
            .get_player_mut(survivor_update.id)
            .expect("Generated id in range 0..TEAM_MAX_CAPACITY always less than TEAM_MAX_CAPACITY.");
    ...
    fn view_team(&self) -> Element<Message> { 
        ...
        for player_id in 0..k::TEAM_MAX_CAPACITY {
            ...
            let (attempt_chance, total_chance) = self.widgets.odds.get(player_id).expect("Generated id in range 0..TEAM_MAX_CAPACITY always less than TEAM_MAX_CAPACITY.");
            let row_output = row![text(attempt_chance.to_owned()), text(total_chance.to_owned())];
     ...
     fn make_player(&self, id: usize) -> Element<Message> {
        let player = self.team.get_player(id).expect("Generated id in range 0..TEAM_MAX_CAPACITY always less than TEAM_MAX_CAPACITY.");
...
impl WidgetData {
    fn renew_odds(&mut self, team: &team::Team) {
        ...
        self.odds = team.luck_output().into_iter()
...
impl TierSlotDisplay {
    fn total_combo_box() -> combo_box::State<Self> {
        combo_box::State::new(perk::Tier::iterator()
...
impl OfferingSlotDisplay {
    pub fn total_combo_box() -> combo_box::State<Self> {
        combo_box::State::new(offering::Offering::iterator()
...
impl SurvivorUpdate {
    fn slippery(id: usize, tier: Option<perk::Tier>) -> SurvivorUpdate {
        SurvivorUpdate {
            id,
            update: SurvivorUpdateData::Perk(
                PerkUpdate {
